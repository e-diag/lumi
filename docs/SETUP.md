...
Поднимутся: **postgres**, **api**, **bot**, **web**. Порты: `5432`, `8080`, `3000`. Все три бинаря читают один **`config.yaml`** из образа и переменные из **`.env`**. В `docker-compose.yml` для контейнеров заданы **`DATABASE_DSN`** с хостом **`postgres`** и **`PGPASSWORD`** из **`POSTGRES_PASSWORD`** (пароль не кладётся в URL — так безопаснее для спецсимволов).
