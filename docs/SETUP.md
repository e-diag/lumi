...
| Переменная | Описание |
|------------|----------|
| `DATABASE_DSN` | DSN PostgreSQL для локального запуска, например `postgres://user:pass@localhost:5432/freeway?sslmode=disable`. В **docker-compose** хост и `PGPASSWORD` задаются в YAML (DSN без пароля в URL). |
...
| Переменная | Описание |
|------------|----------|
| `POSTGRES_PASSWORD` | Пароль пользователя `freeway` в контейнере PostgreSQL и для приложений (через `PGPASSWORD`). По умолчанию в compose: `password`. **После первого `up` смена пароля в `.env` не меняет уже созданный том** — см. раздел «Типовые ошибки». |
...
API слушает `server.host`:`server.port` из конфига (по умолчанию `0.0.0.0:8080`), веб — `web.host`:`web.port` (например `:3000`). Без **`worker`** локально не будут крутиться фоновые задачи (истечение подписок, опрос ЮKassa, проверки нод и т.д.).
...
Поднимутся: **postgres**, **api**, **worker**, **bot**, **web**. Порты: `5432`, `8080`, `3000` (у `worker` портов нет — только фоновые циклы). Те же **`config.yaml`** и **`.env`**, что и у API. Сервис **`worker`** снимает нагрузку воркеров с HTTP-процесса `api`.
...
**Важно:** в `.env` задайте **`POSTGRES_PASSWORD`** (один раз, до первого запуска с пустым томом, либо после согласования с уже существующим паролем в БД). Compose сам подставит DSN и `PGPASSWORD`; строка `DATABASE_DSN` с `localhost` в `.env` нужна в основном для локального `go run`.
...
| `password authentication failed` / **SQLSTATE 28P01** для `freeway` | Пароль приложения ≠ паролю в уже созданном томе PostgreSQL. Образ Postgres **не перезаписывает** пароль при смене `POSTGRES_PASSWORD` в `.env`. Варианты: вернуть в `.env` тот пароль, с которым том создавали; или `docker compose down`, удалить volume `postgres_data`, снова `up` (**данные БД пропадут**); или зайти в `psql` и выполнить `ALTER USER freeway PASSWORD '...';` под суперпользователем. |
| API не коннектится к БД в Docker | Хост в DSN внутри контейнера — `postgres`; один источник пароля — `POSTGRES_PASSWORD` + `PGPASSWORD` в compose |
...
- Масштабирование: **API** stateless; **воркеры** — отдельный контейнер/процесс **`worker`** (один экземпляр на БД, иначе дублируются опросы платежей и health-check). При нескольких репликах API горизонтально масштабируйте только `api`.
...
```
cmd/api          # HTTP API
cmd/worker       # фоновые воркеры (платежи, подписки, ноды, routing)
cmd/bot          # Telegram
cmd/web          # Панель
